...
class Net(nn.Module):
    def __init__(self):
        ...
        self.map_net1 = MapNet()
        self.map_net2 = MapNet()
        self.m2m = M2M()
        self.a2m = A2M()
        self.m2a = M2A()
        self.a2a = A2A()
    ...
    def forward(self, data):
        ...
        lanes1, lane_idcs1 = lane_gather(gpu(data['nbr_waypoints']))
        lane_ctrs1 = gpu(data['nbr_waypoints'])
        lanes1 = self.map_net1(lanes1)
        
        
        # construct map features 2
        lanes2, lane_idcs2 = lane_gather(gpu(data['lane_list']))
        lane_ctrs2 = gpu(data['lane_list'])
        lanes2 = self.map_net2(lanes2)
        
        # actor-map fusion cycle 
        lanes = self.m2m(lanes1, lane_idcs1, lane_ctrs1, lanes2, lane_idcs2, lane_ctrs2)
        lanes = self.a2m(lanes, lane_idcs1, lane_ctrs1, actors, actor_idcs, actor_ctrs)
        actors = self.m2a(actors, actor_idcs, actor_ctrs, lanes, lane_idcs1, lane_ctrs1)
...
class M2M(nn.Module):
    ...
    def forward(self, lanes1, lane_idcs1, lane_ctrs1, lanes2, lane_idcs2, lane_ctrs2):
        for i in range(len(self.att)):
            feat = self.att[i](
                lanes1,
                lane_idcs1,
